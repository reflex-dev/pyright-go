 ...
 * compare-ast.js
 *
 * Differential test for the parser: parses every file in a corpus with both the
 * original TypeScript parser and the Go port, and reports any difference in the
 * resulting parse tree, diagnostics, imported-module list, future imports or
 * typing aliases.
 *
 * This is the parser-level counterpart of compare-corpus.js. Where the unit
 * tests pin the handful of behaviors pyright's authors chose to assert, this
 * pins the whole tree shape -- every node type, every range, every flag -- over
 * real Python source.
 *
 * Two normalizations are applied before comparing, both because Go has zero
 * values where TypeScript has `undefined`:
 *
 *   - A missing property is equal to `false`.
 *   - A missing property is equal to an empty array.
 *
 * Nothing else is normalized: node types, ranges, string contents and numeric
 * values must match exactly.
 *
 * Usage:
 *   node compare-ast.js --ref <pyright-internal/src> \
 *                       --server <tokenserver> \
...
const outDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pyright-go-ast-'));
...
import { dump } from ${JSON.stringify(path.join(bridgeDir, 'dump-ast.ts'))};
...
// AST dump's import chain reaches common/uri/uri.ts, which imports vscode-uri.
...
const requests = files
    .map((file) => JSON.stringify({ op: 'parse', text: codeUnits(fs.readFileSync(file, 'utf8')) }))
    .join('\n');
