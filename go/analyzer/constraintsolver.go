...
// assignBoundTypeVar corresponds to the function of the same name. The
// original's comment: handles an assignment to a TypeVar that is "bound" rather
// than "free". In general such assignments are not allowed, but there are some
...
func assignBoundTypeVar(
	...
	noteConstraintSolverUnported(evaluator, "constraintSolver.assignBoundTypeVar")
	return false
}

// assignUnconstrainedTypeVar corresponds to the function of the same name: the
// ordinary case, where the assignment narrows the TypeVar's bounds.
func assignUnconstrainedTypeVar(
	...
	noteConstraintSolverUnported(evaluator, "constraintSolver.assignUnconstrainedTypeVar")
	return false
}

// assignConstrainedTypeVar corresponds to the function of the same name: the
// value-constrained case, which selects one constraint rather than narrowing.
func assignConstrainedTypeVar(
	...
	noteConstraintSolverUnported(evaluator, "constraintSolver.assignConstrainedTypeVar")
	return false
}
...
func assignParamSpec(
	...
	noteConstraintSolverUnported(evaluator, "constraintSolver.assignParamSpec")
	return false
}
...
func stripLiteralValueForUnpackedTuple(evaluator TypeEvaluator, t Type) Type {
	noteConstraintSolverUnported(evaluator, "constraintSolver.stripLiteralValueForUnpackedTuple")
	return t
}
